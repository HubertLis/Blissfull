defmodule BlissfullySewnWeb.CoreComponents do
  @moduledoc """
  ...
  import BlissfullySewnWeb.Gettext
  ...
  attr :type, :string,
    ...
    values: ~w(checkbox color date datetime-local email file hidden month number password
               range radio search select tel text textarea time url week checkgroup checkgroup2)
  ...
  def input(%{type: "checkgroup2"} = assigns) do
    ~H"""
    <div phx-feedback-for={@name} class="text-sm">
      <.label for={@id}><%= @label %></.label>
      <div class="mt-1 w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        <div class="grid grid-cols-1 gap-1 text-sm items-baseline">
          ...
          <div class="flex items-center" :for={{label, value} <- @options}>
            ...
            <input type="number" name={"#{@name}-#{value}-decimal"} class="ml-2 border rounded p-1" step="0.01" />
          </div>
        </div>
      </div>
      <.error :for={msg <- @errors}><%= msg %></.error>
  ...
  def checkgroup2(assigns) do
    new_assigns =
      assigns
      |> assign(:multiple, true)
      |> assign(:type, "checkgroup2")
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(BlissfullySewnWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(BlissfullySewnWeb.Gettext, "errors", msg, opts)
    end
  end
